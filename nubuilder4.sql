...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 07, 2016 at 02:15 AM
...
('57f5c440a9d27b4', 'nuform', 'lookup', 'sfo_before_open_zzzzsys_php_id', 'Before Open', '5707303145b7760', 240, 223, 927, 100, 18, '1', 'right', 'none', '0', 'SELECT 0', '', '', '', '', '', '', '', '0', '0|No|1|Yes', 'sph_code', 'sph_description', '250', '', 'nuphp', '', '', '', '', '', '', '', '', 'text', '');
...
('57f5cfa9299abcd', '1', '1', '1', '\n\n$pre	= ''sfo_'';\n$newsuf	= ''_zzzzsys_php_id'';\n$oldsuf	= ''_php_id'';\n\n$event	= array(''before_browse'',''before_open'',''before_save'',''after_save'',''before_delete'',''after_delete'');\n\n\nfor($i = 0 ; $i < count($event) ; $i++){\n\n	$e	= $event[$i];\n	$f	= $pre . $e . $oldsuf;\n\n	$s	= "SELECT $f AS event, zzzzsys_form.* FROM zzzzsys_form WHERE $f = '''' OR $f IS NULL";\n	$t	= nuRunQuery($s);\nprint $s . ''<br>'';	\n	\n	while($r = db_fetch_object($t)){\n\n		if($r->event == ''before_browse''){addEvent(''BB'', $r);}\n		if($r->event == ''before_open''){addEvent(''BO'', $r);}\n		if($r->event == ''before_save''){addEvent(''BS'', $r);}\n		if($r->event == ''after_save''){addEvent(''AS'', $r);}\n		if($r->event == ''before_delete''){addEvent(''BD'', $r);}\n		if($r->event == ''after_delete''){addEvent(''AD'', $r);}\n	\n	}\n\ndie();\n}\n\n\n\nfunction addEvent($pre, $row){\n\n	$i	= ''nu'' . nuID();\n	$c	= $pre . ''_''          . $row->sfo_table_name;\n	$d	= $pre . '' code for - '' . $row->sfo_code;\n	$q	= "\n	\n		INSERT INTO zzzzsys_php_id\n		(\n			zzzzsys_php_id, \n			sph_code,\n			sph_description,\n			sph_group,\n			sph_php\n		)\n		VALUES \n		(\n			''$i'',\n			''$c'',\n			''$d'',\n			''test'',\n			''$row->event''\n		)\n	\n	";\n\nprint $q . ''<br>'';\n	nuRunQuery($q);\n	\n}', '', 'nublank');
