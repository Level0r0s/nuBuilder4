...
console.log(event.type);
		...
		if(event.ctrlKey){
			...
			if(c.indexOf('nuDragObject') != -1 && $('.nuDragSelected').length < 2){
	...
	this.move = function(event) {
		...
		if(m || k == 37 || k == 38 || k == 39 || k == 40){
			...
			if(k == 38){this.moveY = -1;}
			if(k == 39){this.moveX = 1;}
			if(k == 40){this.moveY = 1;}
			if(k == 37){this.moveX = -1;}
	...
	this.canMove = function(event) {
		...
		if(this.left + this.moveX + this.areaLeft() < this.areaLeft())  {return false;}
		if(this.right + this.moveX > this.areaWidth())                  {return false;}
		if(this.top + this.moveY + this.areaTop() < this.areaTop())     {return false;}
		if(this.isReport)                                               {return true;}
		if(this.bottom + this.moveY > this.areaHeight())                {return false;}
	...
	this.canResize = function(event) {
		...
		if(this.right + this.moveX > this.areaWidth())         {return false;}
		if(this.width <= this.moveX && event.keyCode != 39)    {return false;}
		if(this.height <= this.moveY && event.keyCode != 40)   {return false;}
		if(this.isReport)                                      {return true;}
		if(this.bottom + this.moveY > this.areaHeight())       {return false;}
	...
	this.moveSelected = function() {
		...
		for(var i = 0 ; i < s.length ; i ++){
			...
			o       = s[i].style;
			l       = parseInt(o.left) + this.moveX;
			t       = parseInt(o.top)  + (m ? 0 : this.moveY);
			o.left  = l + 'px';
			o.top   = t + 'px';
	...
	this.resizeSelected = function() {
		...
		for(var i = 0 ; i < s.length ; i ++){
			...
			o        = s[i].style;
			w        = parseInt(o.width) + this.moveX;
			h        = parseInt(o.height)  + (m ? 0 : this.moveY);
			o.width  = w + 'px';
			o.height = h + 'px';
	...
	this.resizeBox = function(event) {
		...
		$('#nuSelectBox').css({
			'height'           : event.clientY  + window.scrollY - y,
