...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	$style = strtoupper($style);									// Add a TrueType, OpenType or Type1 font
	if($style=='IB')
		$style='BI';
	if($file=='') {
	   if ($uni) {
//		$file = str_replace(' ','',$family).strtolower($style).'.ttf';
		...
		nudebug($file, $family, str_replace(' ','',$family),strtolower($style),'.ttf');
	   }
	   else {
...
{
	nudebug($this->fontpath.$font,$this->fontpath, $font);
...
function _putfonts()
{
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		// TrueType embedded SUBSETS or FULL
			...
			$this->fonts[$k]['n']=$this->n+1;
			require_once('fonts/ttfonts.php');
			$ttf = new TTFontFile();
